...
<script language="C#" runat="server">
            ...
            if (day_out <= numdays ) // *** check and determine if there are day numbers yet to display
            {
                ...
                for (int c=0; c<7; c++) // *** column loop
                {
                    ...
                    else
                    {
                        if (cell_loc > first_day && day_out <= numdays)  // *** we are past the first day and NOT past the number of days
                        {
                            os=os+"<td><center><a href = 'oa_book_appointment.aspx?&docid=" + Request.Params.Get("docid").ToString() +
                                "&patid=" + Request.Params.Get("patid").ToString() +
                                "&year=" + y +
                                "&month=" + (m - 1) +
    ...
    void validate(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            resultOut.InnerHtml = "Your appointment has been scheduled! An email has been sent.";
        }
        ...
        string strEmail = "Insert into email (Email_Message,Email_Deleted) VALUES (@message, 0)";
        string strConnect="server=AUCKLAND;database=gl1517;uid=gl1517;pwd=MSX44gdnE";
        SqlConnection myConnection = new SqlConnection(strConnect);
        SqlCommand myCommandA = new SqlCommand(strInsert, myConnection);
        SqlCommand myCommandB = new SqlCommand(strEmail, myConnection);

        myCommandA.Parameters.AddWithValue( "@docID", Request.Params.Get("docid").ToString());
        myCommandA.Parameters.AddWithValue( "@patID", Request.Params.Get("patid").ToString());
        myCommandA.Parameters.AddWithValue( "@offID", "1" );
        myCommandA.Parameters.AddWithValue( "@date", selectedDate);   
        myCommandB.Parameters.AddWithValue("@message", "Appointment Scheduled Mail sent for Patient " + Request.Params.Get("patid").ToString() + " with Doctor" +  Request.Params.Get("docid").ToString());
        myConnection.Open();
        numa = myCommandA.ExecuteNonQuery();
                                        ...
                                        <div class="form-actions">
                                            <div class="row">
                                                <div class="offset-md-3 col-md-9">
                                                    ...
                                                    <asp:button runat="server" type="button" class="btn btn-default" Text="Cancel" PostBackUrl="oa_select_patient.aspx"></asp:button>
                                                </div>
                                            </div>
                                        </div>
                                    </form>
