...
<script language="C#" runat="server">
    ...
    void validate(Object sender, EventArgs e)
    {
        ...
        if(uInfo != null)
        {
            ...
            if (Page.IsValid)
            {
                ...
                strSelectA ="SELECT Doc_Email FROM doctor WHERE Doc_Email = @email AND Doc_ID <> @EID";
                strConnect="server=AUCKLAND;database=gl1517;uid=gl1517;pwd=MSX44gdnE";
    ...
    void insertDocTable(HttpCookie uInfo)
    {
        string strInsert;
        string strConnect;
    ...
    void insertLoginTable(HttpCookie uInfo)
        {
            ...
            strSelect ="SELECT Doc_ID FROM doctor WHERE Doc_Email = @email";
            SqlCommand myCommandA = new SqlCommand(strSelect, myConnection);
            myCommandA.Parameters.AddWithValue( "@email", email.Text );
            SqlDataReader objDataReader;
            objDataReader = myCommandA.ExecuteReader();
            if(objDataReader.Read())
                uid = "" + objDataReader["Doc_ID"];
            objDataReader.Close();

            //Insert patient login data into login table
            strInsert ="INSERT into login (Log_User_Name,Log_Pass,Log_EID,Log_User_Type) VALUES (@uName,@uPass,@docID,'D')";
            ...
            myCommandB.Parameters.AddWithValue( "@docID", uid);
